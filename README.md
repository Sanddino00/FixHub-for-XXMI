This is a tool to manage and use the fix tools for Genshin, Star Rail, Wuwa, ZZZ and Hi3

<img width="1250" height="932" alt="Screenshot 2026-05-17 145315" src="https://github.com/user-attachments/assets/1105ccb7-9845-494f-80af-d4d022a8a39a" />
<img width="1244" height="930" alt="Screenshot 2026-05-17 145328" src="https://github.com/user-attachments/assets/394dd62a-3712-413e-91e0-2e26c7c9f53b" />
<img width="1239" height="924" alt="Screenshot 2026-05-17 145340" src="https://github.com/user-attachments/assets/938676d8-449a-4535-be60-13e304a5959f" />

You need python for the scripts
Please check out the original creators of the fix tools and support them if possible. I included the links to the original download pages in the credits section.

Usage:

Select the mod folder for each game you want to use this for in the settings.
Then using it select a folder via the CHoose run path button, selet a fix and click run.
Depending on the fix it will either open a terminal window or an exe. Follow what the fix tells you
After it finishes you can close the terminal/exe and select next folder to fix or next fix to use

Resources:

These are fix tools from different creators for Genshin, Star Rail, Wuwa etc. You can get them all on Gamebanana and Github.

The tools included are for updating the character hashes and/or fixing reflection issues etc and to save your toggle choice

Important:
Depending on if you downloaded the resources.zip containing the newest fixes to my knowlege or got them yourself you need to create a folder named resources in the same folder the exe . In the resources folder make  5 subfolders called: gi, hsr, wuwa, zzz, hi3 and place your fixes in the right folder.

For more info on the FaceFix or ORFixapplier for Complex Mods go here:

https://gamebanana.com/tools/21052

https://gamebanana.com/tools/21138

For more info of the other mods use the links in the Credits section.

Credits:

FaceFix and ORFixApplier for Complex mods made by me.

https://github.com/Sanddino00/Gi-FaceFix

https://github.com/Sanddino00/ORFIX_for_Complex_Mods


I don't own the other fixes so if anyone as a problem with me uploading it please contact me. DC:  ._______._._______.1

The other fixes are made by 

SilentNightSound 

Genshin Fix all character mods    https://gamebanana.com/tools/16433

Gustav0 

hsr_convert_c - Pipeline Convert Fix      https://gamebanana.com/tools/19978

golanah921 and LeoMods_ 

Outline/Reflection fix (ORFix) Applier     https://gamebanana.com/tools/16472

petrascyll

hsr_fix33d - Another v3.3 HSR Fix     https://gamebanana.com/tools/19559

caverabbit
WWMI RabbitFX - Glow FX + Censor Remover   https://gamebanana.com/mods/527815
ZZZ RabbitFX   https://gamebanana.com/mods/531649

Sora
star rail update mods scripts    https://gamebanana.com/mods/631661
and fanbox https://www.fanbox.cc/@sora935647402

NightLancerX 

globalpersistswapkey    https://gamebanana.com/tools/19740

Moonholder

Wuwa_Mod_Fixer    https://github.com/Moonholder/Wuwa_Mod_Fixer/releases

shaojiang

zzz_fix20g_by_hc_python - ZZZ 2.0 Fix; Slot Fix-Texture and Lighting Fixes   https://gamebanana.com/tools/20010#FileInfo_1462243

If I forgot to link something please let me know so I can fix it.
The intel fix for hsr:
https://discord.com/channels/1313561268266733729/1362084819273519498/1437263185974526104

Some of the tools were created by multiple of these people but I didn't put everyone everywhere because it would make the side to long.

Please check out the original creator of the tools and support them if possible 

Feel free to modify it in anyway you like.
