...
def update_program_action():
    ...
    def thread_job():
        try:
            ...
            exe_asset = next((a for a in assets if a.get("name", "").lower() == "fixmanager.exe"), None)
            if not exe_asset:
                messagebox.showinfo("Update", "No fixmanager.exe found in latest release.")
                status_text.set("Ready")
                return

            download_url = exe_asset.get("browser_download_url")
            cur_dir = os.path.dirname(os.path.abspath(sys.executable if getattr(sys, "frozen", False) else __file__))
            tmp_installer = os.path.join(cur_dir, "fixmanager_new.exe")
            ...
            status_text.set("Downloading fixmanager.exe...")
            ok, err = download_to_path(download_url, tmp_installer, progress)
            if not ok:
                messagebox.showerror("Download Failed", err or "Unknown")
                status_text.set("Ready")
                progress_bar_exe['value'] = 0
            ...
            subprocess.Popen([tmp_installer], shell=False, close_fds=True)
            launch_msg = "New version launched."

            progress_bar_exe['value'] = 100
            status_text.set(launch_msg)
            messagebox.showinfo("Update", launch_msg)
            root.destroy()
            os._exit(0)
        except Exception as e:
